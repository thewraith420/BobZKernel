...
cd "$KERNEL_DIR"
KERNELRELEASE=$(make -s kernelrelease 2>/dev/null || echo "6.19.0-BobZKernel")
...
Built: $(date)
Optimizations:
- RSEQ Timeslice Extension (experimental userspace time-slice requests)
- Cluster-aware CPU grouping (optimized NVMe IRQ affinity, 15%+ performance)
...
- **RSEQ Timeslice Extension**: Experimental userspace time-slice requests
- **Cluster-aware CPU grouping**: Optimized NVMe IRQ affinity for 15%+ performance
