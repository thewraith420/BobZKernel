#!/bin/bash
# Build the 7.2 kernel with optimizations
# Usage: ./build-kernel-7.2.sh
...
if [ "$BRANCH" = "generic-build" ]; then
    CONFIG_FILE="$BASE_DIR/configs/config-7.2-generic"
    echo -e "${BLUE}Branch: generic-build - using generic (x86-64) config${NC}"
elif [ "$BRANCH" = "pixel-slate" ] || [ "$BRANCH" = "pixel-slate-7.2" ]; then
    CONFIG_FILE="$BASE_DIR/configs/config-7.2-pixel-slate"
    echo -e "${BLUE}Branch: $BRANCH - using Pixel Slate (camera + audio optimized) config${NC}"
elif [ "$BRANCH" = "nightfall-kernel" ]; then
    CONFIG_FILE="$BASE_DIR/configs/config-7.2-nightfall"
    echo -e "${BLUE}Branch: nightfall-kernel - using minimal Nightfall boot-manager config${NC}"
else
    CONFIG_FILE="$BASE_DIR/configs/config-7.2-march-native"
...
if [ "$BRANCH" = "master" ] || [ "$BRANCH" = "march-native" ] || [ "$BRANCH" = "linux-7.2" ]; then
    ...
    echo -e "${GREEN}✓ x86-64-v2 optimizations will be applied (Nightfall must boot on any PC)${NC}"
else
...
if [ "$BRANCH" = "pixel-slate" ] || [ "$BRANCH" = "pixel-slate-7.2" ]; then
    echo -e "${BLUE}Applying Skylake (Kaby Lake) optimizations for Pixel Slate...${NC}"
    KCFLAGS="-march=skylake -mtune=skylake"
elif [ "$BRANCH" = "nightfall-kernel" ]; then
    # Nightfall has to run on whatever PC it is installed on, so no -march=skylake
    # (it emits BMI2/AVX2-era instructions that SIGILL on older CPUs). x86-64-v2
    # is the same baseline generic-build uses: any Nehalem/Bulldozer or newer.
    echo -e "${BLUE}Applying x86-64-v2 optimizations for the Nightfall boot manager...${NC}"
    KCFLAGS="-march=x86-64-v2 -mtune=generic"
elif [ "$BRANCH" = "master" ] || [ "$BRANCH" = "march-native" ] || [ "$BRANCH" = "linux-7.2" ]; then
...
# Clean up old 7.2 build logs
echo -e "${BLUE}Cleaning old 7.2 build logs...${NC}"
find "$BASE_DIR" -maxdepth 1 -name "build-7.2-*.log" -type f -delete 2>/dev/null || true
echo -e "${GREEN}✓ Old logs removed${NC}"

# Set up log file
LOG_FILE="$BASE_DIR/build-7.2-$(date +%Y%m%d-%H%M%S).log"
...
echo ""
if [ "$BRANCH" = "nightfall-kernel" ]; then
    # Nightfall's initramfs has no modprobe and never loads a module, so
    # there is nothing on the target to build against these headers -
    # skip packaging them rather than ship ~700MB nothing will ever use.
    echo -e "${YELLOW}⊘ Skipping headers packaging - nightfall-kernel's initramfs never loads a module${NC}"
else
