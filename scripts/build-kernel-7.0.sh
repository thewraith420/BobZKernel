#!/bin/bash
# Build the 7.0 kernel with optimizations
# Usage: ./build-kernel-7.0.sh
...
if [ "$BRANCH" = "master" ] || [ "$BRANCH" = "march-native" ] || [ "$BRANCH" = "linux-7.0" ]; then
...
elif [ "$BRANCH" = "master" ] || [ "$BRANCH" = "march-native" ] || [ "$BRANCH" = "linux-7.0" ]; then
...
# Clean up old 7.0 build logs
echo -e "${BLUE}Cleaning old 7.0 build logs...${NC}"
find "$BASE_DIR" -maxdepth 1 -name "build-7.0-*.log" -type f -delete 2>/dev/null || true
echo -e "${GREEN}✓ Old logs removed${NC}"

# Set up log file
LOG_FILE="$BASE_DIR/build-7.0-$(date +%Y%m%d-%H%M%S).log"
