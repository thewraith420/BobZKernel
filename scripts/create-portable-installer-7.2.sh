...
case "$BRANCH" in
    ...
    nightfall-kernel)
        ...
        ARCH_NOTE="Nightfall boot-manager kernel: runs on any PC (also the Pixel Slate). Secure Boot must be off."
        EXPECTED_VENDOR="any"
...
if [ -f "$HEADERS_TARBALL" ]; then
    ...
    echo -e "${BLUE}No headers bundled - deliberate on nightfall-kernel, its initramfs never loads a module${NC}"
else
    echo -e "${YELLOW}⚠ No linux-headers-${KERNELRELEASE}.tar.gz found - build-kernel-7.2.sh produces${NC}"
