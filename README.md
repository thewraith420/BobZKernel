# BobZKernel — Optimized Linux 7.1 (workpc variant)

This is the **`workpc`** variant of BobZKernel — Linux 7.1.x tuned for **AMD FX-series Piledriver/Vishera (family 15h) desktops** with a Radeon GPU on the `radeon` kernel driver. Compiled with `-march=bdver2 -mtune=bdver2` so the kernel can be cross-built on a different machine (e.g. the Lenovo laptop) and still run native on the AMD target.

For the multi-variant project overview (laptop, Pixel Slate, generic, ...) see the [master branch README](https://github.com/thewraith420/BobZKernel/tree/master). For just the binary for this variant, see [v7.1.0-workpc](https://github.com/thewraith420/BobZKernel/releases/tag/v7.1.0-workpc).
...
## Target Hardware (workpc variant)

Verified on:

- **Motherboard**: ASUS M5A78L-M LX PLUS (AM3+, AMD 760G + SB710 chipset)
- **CPU**: AMD FX-4300 Quad-Core (Vishera/Piledriver, family 15h)
- **GPU**: Integrated AMD Radeon RS780L (HD 3000-series, R600/TeraScale 1 — driven by `radeon`, NOT `amdgpu`)
- **RAM**: 8 GB DDR3
- **OS**: Linux Mint 22.3 (Ubuntu 24.04 LTS base)

Should also work on similar AMD Family 15h hardware (Bulldozer, Piledriver, Steamroller, Excavator) — the `bdver2` codegen target is a strict subset of everything from Excavator forward including AMD Zen.
...
After reboot on a workpc-class machine:

```bash
uname -r
# 7.1.0-BobZKernel-workpc (or current point release)

# LTO + generic CPU config (codegen is bdver2 via KCFLAGS, not via .config)
...
# Radeon driver loaded (built into the kernel for early KMS)
